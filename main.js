...
async function runGame(plans, Display) {                                                 // là 1 hàm async
    ...
    for (var level = 0; level < 5 && lives >= 0;) {                           // vòng lặp game
    ...
    window.addEventListener("keydown", async (event) => {
        if(event.key == "y" || event.key == "Y") {
            ...
            for (;lives >= 0;) {                           
                ...
                document.getElementById("live").innerHTML = `: ${lives}`;                        // hiển thị text
                let status = await runLevel(new Level(plans[5]),Display);                    // biến trạng thái (won or lost) nhận từ Promise của  runLevel (await: đợi đến khi nào Promise trả về trạng thái mới chạy tiếp)
                if (status == "won") {
                    break;
                } else {                                                                         // status == "lost"
                    --lives;
